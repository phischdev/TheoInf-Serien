...
\item Kl[3] = $\{cxcy \mid x \in \{a,b\}^*, y \in \{a,b,c\}^*\}$
\item Kl[4] = $\{cxcyc \mid x \in \{a,b\}^*, y \in \{a,b,c\}^*\}$
\item Kl[5] = $\{xy \mid x \in \{a,b\}, y \in \{a,b,c\}^*\}$
\end{enumerate}

Die beiden endlichen Automaten können nach der Methode des modularen Entwurfs (Konstruktion eines Produktautomaten) zu folgendem EA kombiniert werden. (Die Zustände B1 und C1 können nie erreicht werden und könnten somit gelöscht werden. Ebenso können deswegen die Zustände A5, B5 und C5 in einen kombiniert werden. Wir belassen sie aber, damit man den Zusammenhang zu den beiden Teilautomaten besser sieht.)
