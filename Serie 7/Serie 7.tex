...
\begin{homeworkProblem}
	...
	\[
		...
		\Bigr\}
	\]
	Schrittweise (ab $\cent$ nach rechts) um in $\cent\, 0^i,\;i\in \mathbb{N}$, wie folgt:
	\begin{enumerate}[1.]
		\item
	...
	\end{enumerate}
	Wiederhole Schritte bis Kopf Zeichen $\textvisiblespace$ liest. Dann akzeptiere.\\
	Es steht nun genau $f(n)\cdot g(n) := h(n)$, (`$\cdot$' ist Konkatenation) auf dem Band und der benutzte Platz während der Berechnung der Simulation der beiden Turingmaschinen ist $\Space_H = \max\{|f(n)|,\, |g(n)|\}$ und nach dem Schreiben des Ergebnisses $\Space_H =|f(n)|+|g(n)| = |h(n)|$.\\
