...
\begin{homeworkProblem}
	\begin{enumerate}[(a)]
		...
		& = 3\sum_{i=0}^{n-2}fib_{i} & (fib_0 = 0) \\
		& = 3(fib_{n}-1). & (\sum_{i=0}^{n}fib_{i} = fib_{n+2}-1)
		\end{align*}
		$3(fib_n-1) = 3fib_n -1 \in \mathcal{O}(fib_n)$. Damit ist $f(n)$ zeitkonstruierbar. 
	\end{enumerate}
\end{homeworkProblem}
\begin{homeworkProblem}
	...
	Ist der Lesekopf des Arbeitsbandes auf dem Symbol $\icol{\alpha\\\beta}$, simuliert $H$ $F$ so als würde $F$ $\alpha$ lesen. Schreibt $F$ ein neues Zeichen $\alpha'$, schreibt $H$ $\icol{\alpha'\\\beta}$ auf das Band.\\
