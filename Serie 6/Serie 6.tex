...
\begin{homeworkProblem}
	...
	\begin{center}
		...
		\draw[thick,->] (5.7,2) to node [align=right, scale=0.75] {$w'=\text{Kod}(M)\#x\#0^i$} (8.7,2);
		...
		\draw[thick,-] (1.3,0.5) to node [align=left]{$w\not=\text{Kod}(M)\#w$} (11.5,0.5);
		\draw[thick,->] (11.5,0.5) -- (11.5,1.5);
		
		\draw (1.2,2) node {$Syntax$};
	...
	Falls die Anzahl Zustände de TM $M$ nicht $\geq i+1$ ist, verwerfen wir w. (Diese Arbeit führt der Algorithmus $A$ aus.)
	Ansonsten fahren wir fort, wie folgt:
...
\begin{homeworkProblem}
	...
	\[
		...
		\left\{ 
			\begin{array}{lll}
				w \not= \mbox{Kod}(M)\#x\#0^i \\
				w     = \mbox{Kod}(M)\#x\#0^i \mbox{ und M hat weniger als $i+1$ Zustände}\\
				w     = \mbox{Kod}(M)\#x\#0^i \mbox{, M hat mehr als $i$ Zustände, M erreicht den $i$-ten Zustand nicht}\\
			\end{array}
		\right.
	\]
	...
	\begin{center}
		...
		\draw[thick,->] (3,2.5) to node [above, scale=0.75] {$w=\text{Kod}(M)\#x$} (5,2.5);
		\draw[thick,->] (3,1.5) to node [align=left, scale=0.75] {Nummer des\\Zustands $q_{accept}$} (5,1.5);
		\draw[thick,->] (7,2) to node [align=center, scale=0.75] {$w'=$\\$\text{Kod}(M)\#x\#0^i$} (9,2);
		
		\draw[thick,->] (11,2.5) to node [align=right, scale=0.9] {akzeptiert $w'$\\ $w \in L_u$} (13,2.5);
		\draw[thick,->] (11,1.5) to node [align=right, scale=0.9] {verwirft $w'$\\$w \notin L_u$} (13,1.5);
		
		\draw[thick,-] (2,1) -- (2,0.5);
		\draw[thick,-] (2,0.5) -- (11.5,0.5);
		\draw[thick,-] (11.5,0.5) -- (11.5,1.5);
		
		\draw (2,2) node {$Syntax$};
		\draw (6,2) node {$concat$};
		\draw (10,2) node [scale=0.9] {$L(M_{q_i})=L_{q_i}$};
		\draw (6,4.35) node {Algorithums $B$ für $L_U$};
