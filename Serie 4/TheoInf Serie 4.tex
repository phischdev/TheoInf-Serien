...
\begin{homeworkProblem}
	\begin{enumerate}[(a)]
		...
		\begin{center}
			...
			\node[state]					(12) [below of=2]		{$p_0$};
			\node[state]					(4) [below of=12]		{$p_1$};
			\node[state]					(5) [below left=1.25 cm and 3 cm of 4]	{$p_{10}$};
			\node[state]					(6) [below right=1.25 cm and 3 cm of 4]	{$p_{11}$};
			\node[state]					(7) [below left of=5]	{$p_{100}$};
			\node[state]					(8) [below right of=5]	{$p_{101}$};
			\node[state]					(9) [below left of=6]	{$p_{110}$};
			\node[state]					(10) [below right of=6]	{$p_{111}$};
			\node[state,accepting]			(11) [below=6cm of 4]	{$p_{1y1}$};
			
			\path	(1)		edge 							node {1}	(2)
			edge [loop above]				node {0}	(1)
			edge [line width=1.25pt]		node {0}	(12)
			edge [line width=1.25pt,bend right=10,left]	node {1}	(4)
			(2)		edge 							node {1}	(3)
			edge [loop above]				node {0}	(2)
			edge [line width=1.25pt]		node {0}	(12)
			edge [line width=1.25pt,bend left]		node {1}	(4)
			(3)		edge [bend left,above left]		node {1}	(1)
			edge [loop above]				node {0}	(3)
			edge [line width=1.25pt]		node {0}	(12)
			edge [line width=1.25pt,bend left=10]		node {1}	(4)
			(4)		edge 							node {0}	(5)
			edge 							node {1}	(6)
			(5)		edge 							node {0}	(7)
			edge [bend left=20]				node {1}	(8)
			(6)		edge 							node {0}	(9)
			edge 							node {1}	(10)
			(7)		edge 							node {1}	(11)
			(7)		edge [bend left=20]				node {0}	(12)
			(8)		edge 							node {1}	(11)
			(8)		edge [bend left=20]				node {0}	(5)
			(9)		edge 							node {1}	(11)
			(9)		edge [bend left=20]				node {0}	(7)
			(10)	edge 							node {1}	(11)
			(10)	edge 							node {0}	(9)
			(11)	edge [loop below]				node {0,1}	(11)
			(12)	edge [loop left]				node {0}	(12)
			(12)	edge 							node {1}	(4)
			;
			\end{tikzpicture}
		\end{center}
		
		Die Idee des Entwurfs ist, dass der NEA aus zwei Teil-EA besteht, die je folgende Bedingungen prüfen:
		...
		Der EA, der Bedingung (1) überprüft, ist durch die Zustände $\{q_0,q_1,q_2\}$ gegeben und derjenge, der (2) überprüft, durch die Zustände $\{p_0,p_1,p_{10},p_{11},p_{100},p_{101},p_{110},p_{111},p_{1y1 \mid y \in \{0,1\}^2}\}$.
		
		Verbunden sind die beiden EA durch die fett markierten Pfeile, wodurch ein NEA entsteht.
		
		Der NEA kann sich in jedem der Zustände $q_0,q_1,q_2$ entscheiden, ob er statt Bedingung (1) nun Bedingung (2) prüfen will. Hat er sich entschieden, Bedingung (2) zu prüfen, kann er nicht mehr zur Prüfung von Bedingung (1) zurückgehen, da er nicht mehr die Anzahl Einsen zählt.
